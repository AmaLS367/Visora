...
- **Read-Only Prefab Override Inspection:** new `PrefabOverrideInspectionService` and `POST /api/visora/prefab/overrides`, advertising the `prefab_override_inspection` capability flag. Diffs a scene Prefab instance (nearest or outermost root) against its source asset using `GetObjectOverrides`, `GetPropertyModifications` + `IsDefaultOverride`, `GetAdded/RemovedComponents`, and `GetAdded/RemovedGameObjects`; returns typed values and structured object references, source-chain apply targets with Model/immutable exclusions, SHA-256-based stable override IDs with collision reporting, deterministic ordering, and `maxOverrides` truncation with real totals. Never applies, reverts, saves, selects, dirties, or records Undo.
- **Strict Hierarchy Path Resolution:** new shared `HierarchyPaths` helper resolves root-anchored paths across all loaded scenes with indexed `Name[k]` segments, and reports missing and ambiguous paths explicitly instead of using `GameObject.Find`. `PrefabInspectionService` now uses the same segment convention helper.
