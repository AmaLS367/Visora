...
- **AnimationClip and event authoring MCP tools:** 9 typed MCP tools for non-destructive animation clip editing:
  ...
  - Dual-path parity: native HTTP endpoints under `/api/visora/animation/*` with feature detection (`animation_authoring`) and self-contained legacy C# script fallback with extracted shared algorithms.
